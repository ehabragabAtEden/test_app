...
class RatingBarWidget extends StatefulWidget {
  ...
  const RatingBarWidget.readOnly({
    Key? key,
    required this.filledIcon,
    required this.emptyIcon,
    this.maxRating = 5,
  ...
  @override
  State<RatingBarWidget> createState() => _RatingBarState();
}

class _RatingBarState extends State<RatingBarWidget> {
  ...
  Widget _buildIcon(int position) {
    if (widget._readOnly) {
      return _buildIconView(position);
    }
  ...
  Widget _buildIconView(int position) {
    ...
    if (widget._readOnly) {
      ...
      final currentRating = _currentRating;
      if (currentRating == null) {
        throw AssertionError('rating can\'t null');
      }
    ...
    return SizedBox(child: iconData);
  }
}
