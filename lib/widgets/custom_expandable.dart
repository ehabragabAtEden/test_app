...
class ExpandableThemeData {
  static const ExpandableThemeData defaults = ExpandableThemeData(
    ...
    scrollAnimationDuration: Duration(milliseconds: 250),
    crossFadePoint: 0.5,
  ...
  /// where the [/collapsedWidget] and [/expandedList] widgets are half-visible.
  ///
  /// If set to 0, the [/expandedList] widget will be shown immediately in full opacity
  ...
  ///
  /// If set to 0.5, the [/expandedList] and the [/collapsedWidget] widget will be shown
  ...
  ///
  /// If set to 1, the [/expandedList] widget will be shown at the very end of the size animation.
  ///
  /// When collapsing, the effect of this setting is reversed. For example, if the value is 0
  /// then the [/expandedList] widget will remain to be shown until the end of the size animation.
...
class Expandable extends StatelessWidget {
  ...
  final bool withCollapsedHeader;
  ...
  const Expandable({
    Key? key,
    required this.withCollapsedHeader,
...
class ExpandablePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget buildHeaderRow() {
      ...
      if (!theme.hasIcon!) {
        return wrapWithExpandableButton(
            widget: headerWidget, wrap: theme.tapHeaderToExpand!);
      } else {
        final rowChildren = <Widget>[
          Expanded(
            child: headerWidget ?? Container(),
          ),
    ...
    Widget buildBody() {
      ...
      final builder = this.builder ??
          (context, collapsed, expanded) {
            return Expandable(
              collapsed: collapsed,
              withCollapsedHeader: withCollapsedHeader,
              expanded: expanded,
              theme: theme,
            );
          };

      return builder(context, wrapBody(collapsedWidget, theme.tapBodyToExpand!),
          wrapBody(expandedList, theme.tapBodyToCollapse!));
    }
        ...
        children: <Widget>[
          Padding(
            padding: EdgeInsets.only(
                ...
                left: SizeConfig.height * 0.002,
                right: SizeConfig.height * 0.002),
            child: buildBody(),
          ),
          Container(
            height: headerHeight,
    ...
    final panel = headerWidget != null ? buildWithHeader() : buildBody();
